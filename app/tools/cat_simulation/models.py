    ...
    try:
        ...
        if unsterilized > 0:
            ...
            if unsterilized == 1:
                ...
                # Assume about 15% are under maturity age
                kitten_count = max(1, int(unsterilized * 0.15)) if unsterilized > 2 else 0
                ...
                if kitten_count > 0:
                    ...
                    if kitten_count <= 3:
                        for _ in range(kitten_count):
                            age = np.random.randint(2, maturity_months)
                            colony['young_kittens'].append([1, age])
                    else:
                        # Use triangular distribution skewed towards older kittens
                        ages = np.random.triangular(2, maturity_months-1, maturity_months, kitten_count)
                        ages = np.clip(ages, 2, maturity_months - 1).astype(int)
                ...
                if adult_count > 0:
                    ...
                    if adult_count <= 3:
                        age_ranges = [(maturity_months, 24), (24, 36), (36, 48)]
                        ...
                        young_adults = max(1, int(adult_count * 0.5))
                        middle_aged = max(1, int(adult_count * 0.3))
                        mature = max(1, int(adult_count * 0.15))
                        ...
                        # Add each age group separately with tighter age ranges
                        if young_adults > 0:
                        ...
                        if mature > 0:
                            age = np.random.randint(36, 48)
                            colony['reproductive'].append([mature, age])
                        if senior > 0:
                            age = np.random.randint(48, 60)
        ...
        if sterilized > 0:
            ...
            if sterilized <= 3:
                ...
                young_adults = max(1, int(sterilized * 0.5))
                middle_aged = max(1, int(sterilized * 0.3))
                mature = max(1, int(sterilized * 0.15))
